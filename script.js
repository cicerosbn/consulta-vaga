...
const dimensoesPorTipo = {
  "Padrão":   "4,5m x 2,3m",
  "Grande":   "5,0m x 2,5m",
  "Paralela": "5,3m x 2,3m",
  "PCD": {
    "15,50": "5,0m x 3,1m",
    "18,50": "5,0m x 3,7m"
  }
...
form.addEventListener("submit", e => {
  ...
  if (!torre) {
    resultado.textContent = "Selecione a torre.";
    return;
  }

  if (!apto) {
    resultado.textContent = "Selecione o apartamento.";
    return;
  }
...
if (registro) {
  ...
  /*const dimensoes = dimensoesPorTipo[tipo];*/
	...
	if (tipo === "PCD") {
	  dimensoes = dimensoesPorTipo.PCD[area];
	} else {
	  dimensoes = dimensoesPorTipo[tipo];
	}  
